    ...
    public class DpiScaleForm : Form
    {
        ...
        protected virtual void OnApplyUseDpiScale()
        {
            if (UseDpiScale)
            {
                ...
                {
                    // 使用系统
                    var result = SetThreadAwarenessContext(DPI_AWARENESS_CONTEXT.DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2);
        ...
        public static DPI_AWARENESS SetThreadAwarenessContext(DPI_AWARENESS_CONTEXT newContext)
        {
            try
            {
                ...
                var awarenss = GetAwarenessFromDpiAwarenessContext(oldContext);
